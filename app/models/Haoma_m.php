...
class Haoma_m extends FOX_Model
{
	...
	public function count_list_haoma_likets($page, $limit,$cityid,$lock=0,$hao_type,$hao_pinpai=0,$title_hao_types=0,$jiage='',$shuwei='',$hao_redian=0,$hao_endst='',$tedians='',$hao_heyust='',$hao_jixiongs='',$list=0,$list_a=0,$list_b=0,$list_c=0,$searchnum,$b,$lock=0,$search_type){
		...
		if($hao_type!=10000 && $hao_type !==false){
	...
	public function get_haoma_lists($page, $limit,$cityid,$lock=0,$hao_type,$hao_pinpai=0,$title_hao_types=0,$jiage='',$shuwei='',$hao_redian=0,$hao_endst='',$tedians='',$hao_heyust='',$hao_jixiongs='',$list=0,$list_a=0,$list_b=0,$list_c=0,$searchnum,$b,$lock=0,$search_type=0)
	{
		
//		echo '<!--';
//		echo var_dump($hao_type);
//		echo '-->';
		...
		$this->db->where('hao_lock >=',$lock)->order_by('hao_jiage','asc');
	...
	public function get_count_by_user($hao_user='',$hao_time=0,$hao_city=0, $hao_type='',$hao_timea,$hao_timeb)
	{
		$this->db->select("id");
		$this->db->from('haoma');
		if(!empty($hao_user)){
        ...
        if($hao_timea){
            $this->db->where('hao_time >',strtotime($hao_timea));
        }
        if($hao_timeb){
            $this->db->where('hao_time <',strtotime($hao_timeb));
        }
