...
(function (factory) {
    if (typeof define === "function" && define.amd) {
        // AMD模式
        define('FoxEditor', ["jquery"], factory);
    } else {
        // 全局模式
...
$(function(){
    ...
    if(isIE6 || isIE7 || isIE8){
        ...
        window.onload = function() {
            ...
            var icons = {
                'icon-FoxEditor-link' : '&#xe800;',
                'icon-FoxEditor-unlink' : '&#xe801;',
                'icon-FoxEditor-code' : '&#xe802;',
                'icon-FoxEditor-cancel': '&#xe803;',
                'icon-FoxEditor-terminal':'&#xe804;',
                'icon-FoxEditor-angle-down':'&#xe805;',
                'icon-FoxEditor-font':'&#xe806;',
                'icon-FoxEditor-bold':'&#xe807;',
                'icon-FoxEditor-italic':'&#xe808;',
                'icon-FoxEditor-header':'&#xe809;',
                'icon-FoxEditor-align-left':'&#xe80a;',
                'icon-FoxEditor-align-center':'&#xe80b;',
                'icon-FoxEditor-align-right':'&#xe80c;',
                'icon-FoxEditor-list-bullet':'&#xe80d;',
                'icon-FoxEditor-indent-left':'&#xe80e;',
                'icon-FoxEditor-indent-right':'&#xe80f;',
                'icon-FoxEditor-list-numbered':'&#xe810;',
                'icon-FoxEditor-underline':'&#xe811;',
                'icon-FoxEditor-table':'&#xe812;',
                'icon-FoxEditor-eraser':'&#xe813;',
                'icon-FoxEditor-text-height':'&#xe814;',
                'icon-FoxEditor-brush':'&#xe815;',
                'icon-FoxEditor-pencil':'&#xe816;',
                'icon-FoxEditor-minus':'&#xe817;',
                'icon-FoxEditor-picture':'&#xe818;',
                'icon-FoxEditor-file-image':'&#xe819;',
                'icon-FoxEditor-cw':'&#xe81a;',
                'icon-FoxEditor-ccw':'&#xe81b;',
                'icon-FoxEditor-music':'&#xe911;',
                'icon-FoxEditor-play':'&#xe912;',
                'icon-FoxEditor-location':'&#xe947;',
                'icon-FoxEditor-happy':'&#xe9df;',
                'icon-FoxEditor-sigma':'&#xea67',
                'icon-FoxEditor-enlarge2':'&#xe98b;',
                'icon-FoxEditor-shrink2':'&#xe98c;',
                'icon-FoxEditor-newspaper':'&#xe904;',
                'icon-FoxEditor-camera':'&#xe90f;',
                'icon-FoxEditor-video-camera':'&#xe914;',
                'icon-FoxEditor-file-zip':'&#xe92b;',
                'icon-FoxEditor-stack':'&#xe92e;',
                'icon-FoxEditor-credit-card':'&#xe93f;',
                'icon-FoxEditor-address-book':'&#xe944;',
                'icon-FoxEditor-envelop':'&#xe945;',
                'icon-FoxEditor-drawer':'&#xe95c;',
                'icon-FoxEditor-download':'&#xe960;',
                'icon-FoxEditor-upload':'&#xe961;',
                'icon-FoxEditor-lock':'&#xe98f;',
                'icon-FoxEditor-unlocked':'&#xe990;',
                'icon-FoxEditor-wrench':'&#xe991;',
                'icon-FoxEditor-eye':'&#xe9ce;',
                'icon-FoxEditor-eye-blocked':'&#xe9d1;',
                'icon-FoxEditor-command':'&#xea4e;',
                'icon-FoxEditor-font2':'&#xea5c;',
                'icon-FoxEditor-libreoffice':'&#xeade;',
                'icon-FoxEditor-quotes-left':'&#xe977;',
                'icon-FoxEditor-strikethrough':'&#xea65;'
            };

            //遍历菜单按钮，替换fontIcon
            $('.FoxEditor-container i').each(function(){
                var elem = this,
                    className = this.className,
                    matchs = className.match(/icon-FoxEditor-[^\s'"]+/);
                if (matchs) {
...
//检测jquery是否正常
if(!$){
	alert('检测到页面没有引用jQuery，请先引用，否则FoxEditor将无法使用。');
    return;
} else if(typeof $ !== 'function' || /^\d+\.\d+\.\d+$/.test($().jquery) === false){
	alert('检测到 window.jQuery 已被修改，FoxEditor无法使用。');
    return;
}
//------------------------------------定义全局变量------------------------------------
var document = window.document,
    ...
    //id前缀
    idPrefix = 'FoxEditor_' + Math.random().toString().replace('.', '') + '_',
    globalNum = 1,
...
$.extend($E, {
    ...
    'consoleLog': function(info){
        if(window.console && window.console.log && typeof window.console.log === 'function'){
            window.console.log('FoxEditor提示：', info);
        }
    },
    ...
    'showTableBorder': function($content){
        $content.find('table').each(function(){
            var $this = $(this),
                mark = 'FoxEditor_table_border_mark',
...
$.extend($E, {
    ...
    'htmlTemplates': {
        //删除table,img的按钮
        'elemDeleteBtn': '<a href="#" class="FoxEditor-elemDeleteBtn"><i class="icon-FoxEditor-cancel"></i></a>',
        'imgResizeBtn': '<div class="FoxEditor-imgResize"></div>',

        //整个编辑器的容器
        'editorContainer': '<div id="FoxEditor-container" class="FoxEditor-container"></div>',
        //菜单容器（加上clearfix）
        'btnContainer': '<div id="FoxEditor-btn-container" class="FoxEditor-btn-container clearfix"></div>',
        //菜单组
        'btnContainerGroup': '<div class="FoxEditor-btn-container-group"></div>',
        //单个菜单按钮（一定要有 herf='#'，否则无法监听blur事件）
        'btn': '<a class="FoxEditor-btn-container-btn FoxEditor-btn-container-btn-default" href="#"></a>', 
        //下拉按钮右侧的小三角
        'btnAngleDown': '<i class="icon-FoxEditor-angle-down" style="margin-left:3px;"></i>',
        //btn tooltip
        'tooltip': '<div class="FoxEditor-toolTip"></div>',
        //btn tooltipContent
        'tooltipContent': '<div class="FoxEditor-toolTip-content">{title}</div>',
        //所有弹出框modal的容器
        'modalContainer': '<div class="FoxEditor-modal-container"></div>',
        //modal（按大小分为4种）
        'modal': '<div class="FoxEditor-modal">{content}</div>',
        'modalBig': '<div class="FoxEditor-modal FoxEditor-modal-big">{content}</div>',
        'modalSmall': '<div class="FoxEditor-modal FoxEditor-modal-small">{content}</div>',
        'modalMini': '<div class="FoxEditor-modal FoxEditor-modal-mini">{content}</div>',
        //modal header
        'modalHeader': '<div class="FoxEditor-modal-header clearfix">' + 
                            '<a href="#" commandName="close" class="FoxEditor-modal-header-close"><i class="icon-FoxEditor-cancel"></i></a>' + 
                            '<b>{title}</b>' + 
                            '<div class="FoxEditor-modal-header-line"></div>' + 
                        '</div>',
        //编辑框的容器
        'txtContainer': '<div class="FoxEditor-textarea-container"></div>',
        //编辑框
        'txt': '<div id="FoxEditor-textarea" class="FoxEditor-textarea" contenteditable="true"><p><br/></p></div>',
        //dropmenu
        'dropMenu': '<ul class="FoxEditor-drop-menu">{content}</ul>',
        //dropPanel
        'dropPanel': '<div class="FoxEditor-drop-panel">{content}</div>',
        //dropPanel-big
        'dropPanelBig': '<div class="FoxEditor-drop-panel FoxEditor-drop-panel-big">{content}</div>',
        //dropPanel-floatItem（即 dropPanel 里面一个一个的块 ）
        'dropPanel_floatItem': '<div class="FoxEditor-drop-panel-floatItem">{content}</div>',
        
        //表情dropPanel的tab容器
        'dropPanel_expression_tab_container': '<div class="clearfix FoxEditor-expression-tab-container">{content}</div>',
        //表情dropPanel中的tab
        'dropPanel_expression_tab': '<div index="{index}" class="FoxEditor-expression-tab">{content}</div>',
        //表情分组的容器
        'dropPanel_expression_group_container': '<div class="FoxEditor-expression-group-container">{content}</div>',
        //表情分组
        'dropPanel_expression_group': '<div index="{index}" class="clearfix FoxEditor-expression-group">{content}</div>',
    ...
    'expressionConfig': {
        'path':'http://www.FoxEditor.com/expressions/',
        'fileNames':[1,100],
        'ext':'.gif'
    }
});
$.extend($E, {
	'getUploadImgComponentForCrossDomain': function(editor){
		var uploadUrl = editor.uploadUrl,
			htmlsUrl = editor.htmlsUrl,
	        fileInputName = 'file',  //服务器端根据这个name获取file
	    ...
	    var content =   '<form id="' + formId + '" method="post" enctype="multipart/form-data" target="' + iframeId + '">'+
	                    '   <p>' +langChoose+ '：<input type="file" accept="image/*" name="' + fileInputName + '" id="' + fileId + '"/><input type="hidden" name="fox_csrf_token" id="fox_csrf_token" value="'+$('#token').val()+'" /></p>' +
	                    '   <p style="display:none;">' +langTitle+ '：<input type="text" id="' + titleTxtId + '" style="width:250px;"/></p>' +
	                    '   <p><button id="' + btnId + '"  type="button" class="FoxEditor-modal-btn">' +langUpload+ '</button></p>' +
	    ...
	    $uploadImg_modal.find('#' + btnId).click(function(e){
	        //检验是否传入uploadUrl配置
	        if(htmlsUrl == null || typeof htmlsUrl !== 'string'){
			...
			if (system.win || system.mac || system.xll||system.ipad) { 
				var $btn = $(this),
					...
					//定义callback事件
					window.uploadImgCallback = $("#"+iframeId+"").load(function(){
						var data = $(window.frames[''+iframeId+''].document.body).find("textarea").html();
						...
						window.uploadImgCallback = undefined;
					});				
				...
				reader.onload = function(e){ // reader onload start
					// ajax 上传图片
					$.post(htmlsUrl, {fox_csrf_token:$('#token').val(),file: e.target.result},function(ret){
						if(ret.img!=''){
							arr = ret.split('|');
							//获取图片url
							url = arr[1];
							editor.command(e, 'insertImage', url);
						}else{
							alert('upload fail');
						}
					},'json');
...
$.extend($E.fn, {
    ...
    'init': function($textarea, options){
        ...
        */
		...
		if(htmlsUrl && typeof htmlsUrl === 'string'){
            editor.htmlsUrl = htmlsUrl;
            //获取跨域上传图片的组件
            editor.uploadImgComponent = $E.getUploadImgComponentForCrossDomain(editor);
        }
...
$.extend($E.fn, {
    ...
    'hideModal': function(){
        this.$modalContainer.find('.FoxEditor-modal:visible').hide();

        //经测试，safari浏览器菜单按钮检测不到blur事件
        //因此，只能强制隐藏dropmenu和droppanel
        this.$btnContainer.find('.FoxEditor-drop-panel:visible').hide();
        this.$btnContainer.find('.FoxEditor-drop-menu:visible').hide();
    },
	...
	'bindPaste': function(uploadUrl){
		...
		$txt.on('paste', function(e){
			var data = e.clipboardData || e.originalEvent.clipboardData,
				items = data.items;
			var foxc='fox_csrf_token='+$('#token').val();
				...
				if(value.type.indexOf('image') > -1){
					...
					reader.onload = function(e){
						...
						if(uploadUrl){
							...
							formData.append( "fox_csrf_token", $('#token').val() );  
...
'underline': {
    ...
    'type': 'btn',
    'hotKey': 'ctrl + u',
    'cssClass':'icon-FoxEditor-underline',
    'command': 'underline '
},
'italic': {
    ...
    'type': 'btn',
    'hotKey': 'ctrl + i',
    'cssClass':'icon-FoxEditor-italic',
    'command': 'italic '
},
'removeFormat': {
    'title': langMenus.removeFormat.title,
    'type': 'btn',
    'cssClass':'icon-FoxEditor-eraser',
...
//     'type': 'btn',
//     'hotKey': 'ctrl,shift + i',
//     'cssClass':'icon-FoxEditor-indent-right',
//     'command': 'indent'
// },
// 'outdent': {
//     'title': langMenus.outdent.title,
//     'type': 'btn',
//     'cssClass':'icon-FoxEditor-indent-left',
//     'command': 'outdent'
// }, 
'unLink': {
    'title': langMenus.unLink.title,
    'type': 'btn',
    'cssClass':'icon-FoxEditor-unlink',
    'command': 'unLink ' 
},
'insertHr': {
    'title': langMenus.insertHr.title,
    'type': 'btn',
    'cssClass':'icon-FoxEditor-minus',
    'command': 'InsertHorizontalRule' 
},
'strikethrough':{
    'title': langMenus.strikethrough.title,
    'type': 'btn',
    'cssClass':'icon-FoxEditor-strikethrough',
    'command': 'StrikeThrough'
},
'blockquote': {
    'title': langMenus.blockquote.title,
    'type': 'btn',
    'cssClass':'icon-FoxEditor-quotes-left',
    'command': 'formatBlock',
...
'justify': {
    ...
    'type': 'dropMenu',
    'cssClass':'icon-FoxEditor-align-left',
    'dropMenu': function(){
        var arr = [],
            ...
            data = [
                {
                    ...
                    'txt': '<i class="icon-FoxEditor-align-left"> ' + langMenus.justify.left.title + '</i>'
                },{
                    //居中
                    'commandName': 'JustifyCenter',
                    'txt': '<i class="icon-FoxEditor-align-center"> ' + langMenus.justify.center.title + '</i>'
                },{
                    //右对齐
                    'commandName': 'JustifyRight',
                    'txt': '<i class="icon-FoxEditor-align-right"> ' + langMenus.justify.right.title + '</i>'
                }
            ];
...
'list': {
    ...
    'type': 'dropMenu',
    'cssClass':'icon-FoxEditor-list-bullet',
    'dropMenu': function () {
        var arr = [],
            temp = '<li><a href="#" customCommandName="${command}">${txt}</a></li>',
            $ul,
            ...
            data = [
                {
                    ...
                    'txt': '<i class="icon-FoxEditor-list-bullet"> ' + langMenus.list.unOrdered.title + '</i>'
                },{
                    //有序列表
                    'commandName': 'InsertOrderedList',
                    'txt': '<i class="icon-FoxEditor-list-numbered"> ' + langMenus.list.ordered.title + '</i>'
                }
            ];
...
'fontSize': {
    ...
    'type': 'dropMenu',
    'cssClass': 'icon-FoxEditor-text-height',
    'command': 'fontSize',
    'dropMenu': function () {
        var arr = [],
            //注意，此处commandValue必填项，否则程序不会跟踪
...
'setHead': {
    ...
    'type': 'dropMenu', 
    'cssClass':'icon-FoxEditor-header',
    'command': 'formatBlock ',
    'dropMenu': function(){
...
'foreColor': {
    ...
    'type': 'dropPanel',
    'cssClass': 'icon-FoxEditor-pencil', 
    //'style': 'color:blue;',
    'command': 'foreColor',
    'dropPanel': function(){
        var arr = [],
...
'createLink': {
    ...
    'modal': function (editor) {
        ...
        var content = '<p>' +langLink+ '：<input id="' + urlTxtId + '" type="text" style="width:200px;"  placeholder="http://"/></p>' +
                        '<p>' +langTitle+ '：<input id="' + titleTxtId + '" type="text" style="width:200px;"/></p>' + 
                        '<p>' +langBlank+ '：<input id="' + blankCheckId + '" type="checkbox" checked="checked"/></p>' +
                        '<p><button id="' + btnId + '" type="button" class="FoxEditor-modal-btn">' +langBtn+ '</button></p>',
            $link_modal = $(
    ...
    'dropPanel': function(editor){
        ...
        if( typeof expressionArr[0] === 'string' ){
            ...
            $(function(){
                ...
                var $tabContainer = $('.FoxEditor-expression-tab-container'),
                    $tabs = $tabContainer.children(),
                    $groupContainer = $('.FoxEditor-expression-group-container'),
...
'insertVideo': {
    ...
    'modal': function(editor){
        ...
        var content = '<p>' +langUrl+ '：<input id="' + txtSrcId + '" type="text" style="width:200px;"  placeholder="http://"/></p>' +
                        ...
                        '<p><button id="' + btnId + '" class="FoxEditor-modal-btn" type="button">' +langBtn+ '</button></p>';
        var $video_modal = $(
                $E.htmlTemplates.modalSmall.replace('{content}', content)
            );
...
'insertImage': {
    ...
    'type': 'modal',
    'cssClass': 'icon-FoxEditor-picture',
    'modal': function (editor) {
        var //上传组件
        ...
        var content =   '<p>' +
                        ...
                        '   <p>' +langUrl+ '：<input id="' + urlTxtId + '" type="text" style="width:200px;" placeholder="http://"/></p>' +
                        '   <p>' +langTitle+ '：<input id="' + titleTxtId + '" type="text" style="width:200px;"/></p>' +
                        '<p><button id="' + btnId + '" type="button" class="FoxEditor-modal-btn">' +langBtn+ '</button></p>' +
                        '</div>',
            $webimg_modal = $(
...
'insertCode':{
    ...
    'modal': function(editor){
        ...
        var content = '<p>' +langTitle+ '：{selectLangs}</p>' +  // selectLangs 待填充语言列表下拉框
                        ...
                        '<button id="' + btnId + '"  type="button" class="FoxEditor-modal-btn">插入</button>',
...
'insertLocation':{
    ...
    'type': 'modal',
    'cssClass': 'icon-FoxEditor-location',
    'modal': function(editor){
        ...
        var content = langCity + '：<input type="text" id="' + txtCityId + '" style="width:60px;"/> ' + 
                      ...
                      ' <button class="FoxEditor-modal-btn" id="' + btnSearchId + '"  type="button">' +langSearch+ '</button>' + 
                      ' <button class="FoxEditor-modal-btn" id="' + btnClearId + '"  type="button">' +langClear+ '</button>' +
                      ' <div id="' + divMapId + '" style="width:100%; height:220px; border:1px solid #ccc; margin:10px 0px;">' +langLoading+ '</div>' +
                      ' <button class="FoxEditor-modal-btn" id="' + btnInsertId +'"  type="button">' +langInsert+ '</button>' +
...
'viewSourceCode': {
    ...
    'modal': function(editor){
        ...
        var content = '<div><textarea style="width:100%; height:200px;" id="' + txtId + '"></textarea></div>' +
                        '<button id="' + btnId + '" class="FoxEditor-modal-btn"  type="button">' +langUpdate+ '</button>';
        ...
        $(function(){
            //注意，这是一步特殊处理！！！
            editor.$btnContainer.find('.icon-FoxEditor-code') //找到<i>
                                .parent()  //找到 <a> 即 btn
            .click(function(e){
...
'fullScreen': {
    ...
    'type': 'btn',
    'cssClass': 'icon-FoxEditor-enlarge2',
    'command': 'fullScreen'
}
		};
	}
	...
	'updateMenuStyle': function(){
	    ...
	    commandBtns.each(function(){
	        ...
	        if($E.commandEnabled(commandName) && document.queryCommandState(commandName)){
	            $btn.addClass('FoxEditor-btn-container-btn-selected');
	        }else{
	            $btn.removeClass('FoxEditor-btn-container-btn-selected');
	        }
	    });
	}
	...
	'commandHooks':{
        ...
        'fullScreen': function(commandName, commandValue){
            var $txtContainer = this.$txtContainer,
                ...
                enlargeClass = 'icon-FoxEditor-enlarge2',
                shrinkClass = 'icon-FoxEditor-shrink2',
...
$.fn.extend({
    ...
    * }
    */
    'FoxEditor': function(options){
	
        if(this[0].nodeName !== 'TEXTAREA'){
            //只支持textarea
            alert('FoxEditor提示：请使用textarea扩展富文本框。');
            return;
        }

        //针对一个textarea不能执行两遍 FoxEditor() 事件
        if(this.data('FoxEditorFlag')){
            alert('针对一个textarea不能执行两遍FoxEditor()事件');
            return;
        }else{
            this.data('FoxEditorFlag', true);
        }
