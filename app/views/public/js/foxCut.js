...
function browserCheck(){
  ...
  var s = $.ua().is360se;
  if(s==true){
    //alert("你正在使用360安全浏览器Chrome内核,内核版本号："+version);
    i=1;
  }
  var s = $.ua().is360ee;
  if(s==true){
    //alert("你正在使用360极速浏览器Chrome内核,内核版本号："+version);
    i=1;
  }
  var s = $.ua().isChrome;
  if(s==true){
    //document.write("你正在使用谷歌浏览器（原版）,内核版本号："+version);
    i=1;
  }
  var s = $.ua().isLiebao;
  if(s==true){
    //document.write("你正在使用猎豹浏览器chrome内核,内核版本号："+version);
    i=1;
  }
  var s = $.ua().isLiebao;
  if(s==true){
  ...
  var s = $.ua().isSougou;
  if(s==true){
    //document.write("你正在使用搜狗浏览器chrome内核,内核版本号："+version);
    i=1;
  }
  var s1 = $.ua().isIe;
  if(s1==true){
    var s2 = $.ua().ie;
    //document.write("你正在使用IE浏览器，内核版本号："+version);
    if(s2<9){
    document.write("您使用的浏览器在IE9.0以下，为了更好地实现网站操作功能，强烈建议您升级您的IE，或者使用其他<a href='https://www.baidu.com/s?wd=安全浏览器'>安全浏览器</a>。推荐<a href='https://www.baidu.com/s?wd=google+chrome'>谷歌浏览器</a>！");
    }
    i=1;
  }
  var s = $.ua().isFirefox;;
  if(s==true){
    //document.write("你正在使用firefox浏览器，内核版本号："+version);
    i=1;
  }
  var s = $.ua().isMobile;
  if(s==true){
    //document.write("你正在使用手机版浏览器");
    i=1;
  }
  var s = $.ua().isTablet;
  if(s==true){
    //document.write("你正在使用平板浏览器");
    i=1;
  }   
  if(i==0){
    //document.write("你正在使用的是"+browser+"浏览器"+"内核版本号："+version);
  }
}