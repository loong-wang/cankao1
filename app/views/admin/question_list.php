			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">													<?php if($question_list && strstr($_SERVER[ 'REQUEST_URI' ],"question/flist")){?>							<form accept-charset="UTF-8" action="<?php echo site_url('admin/question/delall');?>" role="form" method="post" novalidate="novalidate" >							<?php echo csrf_hidden();?>							<?php }?>							
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
									<thead>
										<tr>																						<?php if($question_list && strstr($_SERVER[ 'REQUEST_URI' ],"question/flist")){?>											<th><input id="checkall" type="checkbox" checked="1"></th>											<?php }?>											
											<th>标题</th>
											<th>源自</th>
									...
									foreach($question_list as $k => $v){?>
										<tr>																						<?php if($question_list && strstr($_SERVER[ 'REQUEST_URI' ],"question/flist")){?>											<td>											<input name="<?php echo $k?>" checked="1" value="<?php echo $v['id']?>" type="checkbox">											</td>											<?php }?>											
								...
								<?php }?>																<?php if($question_list && strstr($_SERVER[ 'REQUEST_URI' ],"question/flist")){?>								<input class="btn btn-primary btn-danger" name="batch_del" type="submit" value="批量删除" />								<?php }?>																</form>
							</div><!-- /.span -->
						</div>
			</div><!-- /.main-content -->
...
<?php echo js_url('ace/ace.searchbox-autocomplete');?><script type="text/javascript">$(document).ready(function(){  $("#checkall").bind('click',function(){  $("input:checkbox").prop("checked",$(this).prop("checked"));  });});</script>
</body>
</html>
