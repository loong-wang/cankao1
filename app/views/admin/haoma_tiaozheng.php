...
<?php echo css_url('datepicker');?>
<?php echo css_url('bootstrap-timepicker');?>
</head>
<body class="no-skin">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<?php echo csrf_hidden();?>
									...
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="hao_jiage"> 按价格 </label>
										<div class="col-sm-9">
											<div class='col-xs-10 col-sm-6 col-md-5'>
												<div class="input-group">
													<input type="number" class="col-xs-4 col-sm-4 col-md-3 form-control" id="hao_jiagea" name="hao_jiagea" value="" />
													<span class="input-group-addon">
														<i class="fa fa-exchange"></i>
													</span>
													<input type="number" class="col-xs-4 col-sm-4 col-md-3 form-control" id="hao_jiageb" name="hao_jiageb" value="" />
												</div>
											</div>
										</div>
									</div>
										...
										<div class="col-sm-9">
											<div class='col-xs-10 col-sm-10 col-md-10'>
												<div class="input-daterange input-group">
													<input type="text" class="col-xs-4 col-sm-4 col-md-3 form-control" id="hao_timea" name="hao_timea" value="" />
													<span class="input-group-addon">
														<i class="fa fa-exchange"></i>
													</span>
													<input type="text" class="col-xs-4 col-sm-4 col-md-3 form-control" id="hao_timeb" name="hao_timeb" value="" />
												</div>
											</div>
										</div>
									</div>
									...
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="for_nums"> 调整价格 </label>

										<div class="col-sm-9">
											<input type="number" id="hao_nums" name="hao_nums" placeholder="填写正负数幅度" class="col-xs-10 col-sm-5" value="">
											<span class="help-block col-sm-reset inline col-xs-12 col-sm-7 hidden-480">增减价格</span>
										</div>
									</div>
									<div class="form-group">
										...
										<div class="col-sm-9">
											<div class='col-xs-10 col-sm-5'>
												<span id="hao_numsboxs" class="middle red"></span>
											</div>
										</div>
									</div>
...
<?php echo js_url('date-time/bootstrap-datepicker');?>
<?php echo js_url('date-time/daterangepicker');?>
<!-- ace scripts -->
...
jQuery(function($){
	...
	$('.input-daterange').datepicker({format: 'yyyy-mm-dd',autoclose:true,todayHighlight: true});
...
$(document).ready(function(){
	$("#hao_users").click(function(){ 
		...
		var hao_timea=$("#hao_timea").val();
		var hao_timeb=$("#hao_timeb").val();
		var hao_times='';
		var hao_jiagea=$("#hao_jiagea").val();
		var hao_jiageb=$("#hao_jiageb").val();
		var hao_jiages='';
		if(!hao_user && !hao_city && !hao_timea && !hao_timeb && !hao_jiagea && !hao_jiageb){
			$("#haoboxs").html('');
			...
			if(hao_timea && hao_timeb){
				hao_times=hao_timea+'至'+hao_timeb+' ';
			}else if(hao_timea && !hao_timeb){
				hao_times=hao_timea+'后 ';
			}else if(!hao_timea && hao_timeb){
				hao_times=hao_timeb+'前 ';
			}
			if(hao_jiagea && hao_jiageb){
				hao_jiages=hao_jiagea+'至'+hao_jiageb+' ';
			}else if(hao_jiagea && !hao_jiageb){
				hao_jiages='大于'+hao_jiagea+' ';
			}else if(!hao_jiagea && hao_jiageb){
				hao_jiages='小于'+hao_jiageb+' ';
			}
			if(hao_city){
				hao_citys=hao_citys+' ';
			}
			...
			$.ajax({
				//提交数据的类型 POST GET
				type:"POST",
				url:baseurl+"index.php/admin/haoma/get_hao_by_tz",
				data:{hao_user:hao_user,hao_timea:hao_timea,hao_timeb:hao_timeb,hao_jiagea:hao_jiagea,hao_jiageb:hao_jiageb,hao_city:hao_city,<?php echo $this->config->item('csrf_token_name');?>:token},
				//返回数据的格式
				datatype: "text",//"xml", "html", "script", "json", "jsonp", "text".
				//在请求之前调用的函数
				beforeSend:function(){$('#haobox').html('正在统计 '+hao_citys+hao_users+hao_times+hao_jiages+' 号码……');$('#haoboxs').html('');},
				//成功返回之后调用的函数             
				success:function(data){
					$('#haobox').html('');
					if(data>0){
						$('#haoboxs').html(hao_citys+hao_users+hao_times+hao_jiages+' 号码：'+data+ '个');
						$('#del_num').val(data);
					}else{
						$('#haoboxs').html('没有找到 '+hao_citys+hao_users+hao_times+hao_jiages+' 号码！');
					}
				}       
			});
		}
	...
	$("#del_users").click(function(){ 
		...
		var hao_nums=$("#hao_nums").val();		
		if(del_num==0){
			$("#haobox").html('');
			$("#haoboxs").html('参数有误，请先统计号码数');
		}else if(!hao_nums){
			$("#haobox").html('');
			$("#hao_numsboxs").html('必须填写价格调整幅度');
		}else{
			var token=$('#<?php echo $this->config->item('csrf_token_name');?>').val();
			var hao_user=$("#hao_user").val();
			var hao_city=$("#hao_city").val();
			...
			$.ajax({
				//提交数据的类型 POST GET
				type:"POST",
				url:baseurl+"index.php/admin/haoma/tz_hao_by_tz",
				data:{hao_user:hao_user,hao_timea:hao_timea,hao_timeb:hao_timeb,hao_jiagea:hao_jiagea,hao_jiageb:hao_jiageb,hao_city:hao_city,hao_nums:hao_nums,<?php echo $this->config->item('csrf_token_name');?>:token},
				//返回数据的格式
				datatype: "text",//"xml", "html", "script", "json", "jsonp", "text".
				//在请求之前调用的函数
				beforeSend:function(){$('#hao_numsboxs').val('正在调整……');},
				//成功返回之后调用的函数             
				success:function(data){
					if(data==1){
						$('#hao_numsboxs').html('号码批量调整成功！');
					}else{
						$('#hao_numsboxs').html('号码批量调整失败！');
					}
				}       
			});
		}
