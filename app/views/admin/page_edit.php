			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								<form accept-charset="UTF-8" id="new_add" action="<?php echo site_url('admin/pages/edit/'.$pageinfo['id']);?>" class="form-horizontal" role="form" method="post" novalidate="novalidate">
								<?php echo csrf_hidden();?>
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="pages_title"> 单页标题 </label>

										<div class="col-sm-9">
											<input type="text" id="pages_title" name="pages_title" placeholder="标题" class="col-xs-10 col-sm-5" value="<?php echo set_value('pages_title',$pageinfo['pages_title']);?>">
											<?php if(form_error('pages_title')){?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 red"><?php echo form_error('pages_title');?></span><?php }else{?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 hidden-480">标题</span><?php }?>
										</div>
									</div>
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="pages_type"> 单页类型 </label>
										<div class="col-sm-9">
											<div class="radio">
											<?php if($this->config->item('page_types')){
											foreach(explode("|",$this->config->item('page_types')) as $k => $v){
											?>
											<label>
												<input name="pages_type" type="radio" class="ace" value="<?php echo $k;?>" <?php if(set_checkbox('pages_type',$k) || $pageinfo['pages_type']==$k){echo 'checked';}?> >
												<span class="lbl"> <?php echo $v;?></span>
											</label>
											<?php }}?>
											<?php if(form_error('pages_type')){?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 red"><?php echo form_error('pages_type');?></span><?php }else{?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 hidden-480">类型</span><?php }?>
											</div>
										</div>
									</div>
									...
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="pages_city"> 单页所属 </label>
										<div class="col-sm-9">
											<select class="chosen-select form-control" id="city" data-placeholder="选择城市" onchange="getvalues(this)">
											<option value=""></option>
											<option value="0" <?php if($pageinfo['pages_city']==0){echo 'selected';}?>>全站</option>
											<?php if($citys){
												foreach($citys as $v){?>
												<option value="<?php echo $v['cid'];?>" <?php if($pageinfo['pages_city']==$v['cid']){echo 'selected';}?>><?php echo $v['cname'];?></option>
											<?php }}?>
											</select><input type="hidden" id="pages_city" name="pages_city" value="<?php echo $pageinfo['pages_city'];?>">
											<span class="help-inline col-xs-12 col-sm-7">
												<span class="middle red"><?php echo form_error('pages_city');?></span>
											</span>
										</div>
									</div>
									<?php }else{?>
									<input type="hidden" id="pages_city" name="pages_city" value="<?php echo $this->session->userdata('ucity');?>">
									<?php }?>
									
									
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="pages_content"> 单页内容 </label>
										<div class="col-sm-9">
											<textarea class="form-control" id="post_content" name="content" rows="20"><?php echo $pageinfo['pages_content'];?></textarea>
...
function getvalues(obj)
{
    ...
    $("input[name='pages_city']").attr("value",c);
	if(c){
