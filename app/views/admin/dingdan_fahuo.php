					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<form accept-charset="UTF-8" id="new_add" action="<?php echo site_url('admin/order/fahuo/'.$toid.'/'.$tocity.'/'.$topage);?>" class="form-horizontal" role="form" method="post" novalidate="novalidate">
									...
									<table class="table table-striped table-bordered table-hover">
										<thead>
											<tr>
												...
												<th>收款方式</th>
												<th>快递公司</th>
												<th class="hidden-480">单号/代收人/电话</th>
										...
										<tbody>
											<tr>
												<td><select class="form-control" id="fahuo_type" name="fahuo_type">
													<option value=""></option>
													<?php if($this->config->item('fahuo_type')){
													foreach(explode("|",$this->config->item('fahuo_type')) as $t => $s){?>
													<option value="<?php echo $s;?>" <?php if($dingdan['dan_hao_fahuo_type']==$s){echo 'selected';}?> ><?php echo $s;?></option>
													<?php }}?>
												</select></td>
												<td><select class="form-control" id="shoukuan_type" name="shoukuan_type">
													<option value=""></option>
													<?php if($this->config->item('shoukuan_type')){
													foreach(explode("|",$this->config->item('shoukuan_type')) as $t => $s){?>
													<option value="<?php echo $s;?>" <?php if($dingdan['dan_hao_shoukuan_type']==$s){echo 'selected';}?>><?php echo $s;?></option>
													<?php }}?>
												</select></td>
												<td><select class="form-control" id="kuaidi_type" name="kuaidi_type">
													<option value=""></option>
													<?php if($this->config->item('kuaidi_type')){
													foreach(explode("|",$this->config->item('kuaidi_type')) as $t => $s){?>
													<option value="<?php echo $s;?>" <?php if($dingdan['dan_hao_kuaidi_type']==$s){echo 'selected';}?>><?php echo $s;?></option>
													<?php }}?>
												</select></td>
												<td class="hidden-480"><input type="text" id="fahuo_danhao" name="fahuo_danhao" placeholder="快递单号/代收人/电话" class="form-control" value="<?php echo $dingdan['dan_hao_fahuo_danhao'];?>" ></td>
												<td class="hidden-480"><input type="text" id="fahuo_kuan" name="fahuo_kuan" placeholder="金额" class="form-control" value="<?php echo $dingdan['dan_hao_fahuo_kuan'];?>"></td>
												<td class="hidden-480"><input type="text" id="fahuo_beizhu" name="fahuo_beizhu" placeholder="备注" class="form-control" value="<?php echo $dingdan['dan_hao_fahuo_beizhu'];?>"></td>
											</tr>
										</tbody>
									</table>
									<h4 class="lighter">
