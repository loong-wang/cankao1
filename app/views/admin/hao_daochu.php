						...
						<div class="row">
							<div class="col-xs-12">
								...
								<?php echo csrf_hidden();?>
									...
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="hao_ex"> 生成格式 </label>
										<div class="col-sm-9">
											<div class="radio">
											<label>
												<input name="hao_ex" type="radio" class="ace" value="2003" checked >
												<span class="lbl"> 2003</span>
											</label>
											<label>
												<input name="hao_ex" type="radio" class="ace" value="2007" >
												<span class="lbl"> 2007</span>
											</label>
											</div>
										</div>
									</div>
...
$(document).ready(function(){
	$("#hao_users").click(function(){ 
		...
		if(!hao_user && !hao_city && !hao_timea && !hao_timeb && !hao_type){
			$("#haoboxs").html('');
			...
			$.ajax({
				//提交数据的类型 POST GET
				type:"POST",
				url:baseurl+"index.php/admin/daohao/get_hao_by_dc",
				data:{hao_user:hao_user,hao_timea:hao_timea,hao_timeb:hao_timeb,hao_type:hao_type,hao_city:hao_city,<?php echo $this->config->item('csrf_token_name');?>:token},
				//返回数据的格式
	...
	$("#del_users").click(function(){ 
		...
		if(del_num==0){
			...
			var hao_timeb=$("#hao_timeb").val();
			var hao_type=$('input[name="hao_type"]:checked').val();
			var hao_ex=$('input[name="hao_ex"]:checked').val();
			$.ajax({
				//提交数据的类型 POST GET
				type:"POST",
				url:baseurl+"index.php/admin/daohao/dc_hao_by_dc",
				data:{hao_user:hao_user,hao_timea:hao_timea,hao_timeb:hao_timeb,hao_type:hao_type,hao_city:hao_city,hao_ex:hao_ex,<?php echo $this->config->item('csrf_token_name');?>:token},
				//返回数据的格式
				datatype: "json",//"xml", "html", "script", "json", "jsonp", "text".
				//在请求之前调用的函数
				beforeSend:function(){$('#haobox').val('正在导出……');},
				//成功返回之后调用的函数             
				success:function(data){
					...
					var msg=obj.msg;
					if(ok=='1'){
						$('#haobox').html('号码批量导出成功！');
						$('#haoboxs').html(msg);
					}else{
						$('#haobox').html('号码批量导出失败！');
					}
				}       
			});
		}
