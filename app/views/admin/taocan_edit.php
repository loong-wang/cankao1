			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								<form accept-charset="UTF-8" id="new_add" action="<?php echo site_url('admin/zifei/taocan_edit/'.$taocaninfo['id']);?>" class="form-horizontal" role="form" method="post" novalidate="novalidate">
								<?php echo csrf_hidden();?>
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="tc_title"> 套餐标题 </label>

										<div class="col-sm-9">
											<input type="text" id="tc_title" name="tc_title" placeholder="标题" class="col-xs-10 col-sm-5" value="<?php echo $taocaninfo['tc_title'];?>">
											<?php if(form_error('tc_title')){?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 red"><?php echo form_error('tc_title');?></span><?php }else{?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 hidden-480">标题</span><?php }?>
										</div>
									</div>
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="tc_type"> 套餐类型 </label>
										<div class="col-sm-9">
											<div class="radio">
											...
											?>
											<label>
												<input name="tc_type" type="radio" class="ace" value="<?php echo $k;?>" <?php if(set_checkbox('tc_type',$k) || $taocaninfo['tc_type']==$k){echo 'checked';}?> >
												<span class="lbl"> <?php echo $v;?></span>
											</label>
											<?php }}?>
											<?php if(form_error('tc_type')){?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 red"><?php echo form_error('tc_type');?></span><?php }else{?><span class="help-block col-sm-reset inline col-xs-12 col-sm-7 hidden-480">类型</span><?php }?>
											</div>
										</div>
									</div>
									...
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="tc_city"> 套餐所属 </label>
										<div class="col-sm-9">
											<select class="chosen-select form-control" id="city" data-placeholder="选择城市" onchange="getvalues(this)">
											<option value=""></option>
											<?php if($citys){
												foreach($citys as $v){?>
												<option value="<?php echo $v['cid'];?>" <?php if($taocaninfo['tc_city']==$v['cid']){echo 'selected';}?>><?php echo $v['cname'];?></option>
											<?php }}?>
											</select><input type="hidden" id="tc_city" name="tc_city" value="<?php echo $taocaninfo['tc_city'];?>">
											<span class="help-inline col-xs-12 col-sm-7">
												<span class="middle red"><?php echo form_error('tc_city');?></span>
											</span>
										</div>
									</div>
									<?php }else{?>
									<input type="hidden" id="tc_city" name="tc_city" value="<?php echo $this->session->userdata('ucity');?>">
									<?php }?>									
									
									<div class="space-4"></div>
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="tc_content"> 套餐内容 </label>
										<div class="col-sm-9">
											<textarea class="form-control" id="post_content" name="content" rows="20"><?php echo $taocaninfo['tc_content'];?></textarea>
